...
function setup() {
  ...
  molecules = buildMolecules();
  electric_field = createVector(0, 0);
  selected_molecule = molecules[0];
}

function windowResized() {
  // Remember which molecule was selected, rebuild fresh at new dimensions.
  const selectedIndex = molecules ? molecules.indexOf(selected_molecule) : 0;
  resizeCanvas(windowWidth, windowHeight);
  molecules = buildMolecules();
  selected_molecule = molecules[Math.max(0, selectedIndex)];
}
